...
`103 个自建工具 · 231 个通过的后端单元测试 · 47 条 golden 行为验收 · 30 用户离线推荐评测集`

## 它能做什么

- **图谱问答**：声优 / 制作 / 角色 / 年份 / 跨媒体关系的多跳查询（如"白色相簿2 里冬马和纱的声优还配过哪些高分番"）。
- **个性化推荐**：收藏画像（含评分、时间、搁置/弃坑负信号）+ aspect 好球区/雷区 + 多路召回统一打分；动画、书籍、音乐、游戏、三次元各自使用通过流行度门禁的协同模型，bge 本地模型可用时再启用语义特征，缺失会明确降级；支持对话式修正（"换一批 / 要短的 / 更冷门"）。
...
- **长期记忆**：偏好 / 避雷 / 进度 / 反馈跨会话生效，每条带来源和置信度；`/memory` 可查看、修正、导出或分类清除。
...
- **同好留言与站点脉搏**：Bangumi 登录用户带真实头像公开留言，可跳转个人主页并举报不当内容；原始 IP 不落库，累计访客用不可逆会话哈希去重，逐页明细 48 小时后删除；管理员可在 `/admin` 处理举报并查看推荐质量与任务状态。
...
| 入口 | 说明 |
|---|---|
| **网页** | Live demo 直接开聊；生成中实时显示"它在做什么"，答案带结构化资料卡片；服务端独立运行回答，离开页面后重进可自动重连并回放进度 |
| **手机 (PWA)** | 浏览器打开后"添加到主屏幕"，即是一个 App |
| **Discord** | @机器人 / 私信 / 斜杠命令；`/绑定` 关联 Bangumi 后个性化；回答可通过“在网页继续”或 `/网页续聊` 安全迁移；订阅可推送到私信 |
...
frontend/       Next.js 对话前端：可重连 SSE + 执行过程可视化 + 结构化证据面板 + PWA
...
bash deploy/rollback.sh  # 回到上一个健康部署；也可追加完整 commit SHA
```

推送流水线：push → GitHub Actions 跑测试 → **测试通过才**构建带 commit SHA 的 GHCR 镜像 → 服务器 `bash deploy.sh` 等待并部署该 SHA。源码和容器使用同一个不可变版本；构建失败或超时会保留旧容器，切换后的健康检查失败会自动恢复上一版，不会悄悄运行旧 `latest`。
...
- **推荐进化链**：评分/状态/时间加权反馈 → 流行度/BM25/ALS 同台评测 → 各媒介自动择优 → 只有超过流行度基线才原子发布；五媒介模型均有版本、质量门禁和缺失/过期降级状态。LTR 与语义召回仍保持实验态，不拿小样本结果冒充线上增益。
- **评测**：231 个通过的后端单元测试 + 14 个离线推荐测试 + 47 golden 行为验收（断言路由/面板/越界，不赌易变事实）+ leave-N-out 推荐评测（30 公开用户）+ 五媒介时间/随机留一 CF 评测 + 图谱级 verifier（canonical 对齐/set-F1/幻觉感知）+ LLM 用户模拟器（5 persona 多轮驱动）。
- **数据飞轮**：每轮对话轨迹 + 👍👎 反馈落盘（伪匿名），一键导出脱敏 SFT / DPO 语料，为后训练攒真实分布数据。
- **记忆**：短期会话状态 + SQLite 长期记忆（consolidation 而非追加；多实例三路合并防并发丢写）+ 用户自助查看、修正、导出与清除。
- **RAG**：BM25 + bge dense 混合召回 + reranker 精排（线程池执行不阻塞事件循环）；萌娘 `ai-train=no` 只临时检索、不入库。
- **前端**：Next.js + 可重连 SSE；回答由服务端独立 Chat Run 执行，任务与事件由 SQLite 记账，断开页面仍继续、服务重启则明确标记中断；完整轨迹长期随消息保存。
