...
`103 个自建工具 · 228 个通过的单元测试 · 47 条 golden 行为验收 · 30 用户离线推荐评测集`
...
- **同好留言与站点脉搏**：Bangumi 登录用户带真实头像公开留言，可跳转个人主页并举报不当内容；原始 IP 不落库，累计访客用不可逆会话哈希去重，逐页明细 48 小时后删除。
...
- **评测**：228 个通过的单元测试 + 47 golden 行为验收（断言路由/面板/越界，不赌易变事实）+ leave-N-out 推荐评测（30 公开用户）+ 五媒介时间/随机留一 CF 评测 + 图谱级 verifier（canonical 对齐/set-F1/幻觉感知）+ LLM 用户模拟器（5 persona 多轮驱动）。
...
- **前端**：Next.js + 可重连 SSE；回答由服务端独立 Chat Run 执行，事件短期回放、完整轨迹长期随消息保存；生成中"看它思考"，完成后收敛可回看。
