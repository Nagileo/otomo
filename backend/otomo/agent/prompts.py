...
- 多模态图片路由：用户给 ACGN 图片 URL / data URL / upload:// 并问“这是什么图/出处/哪里来的/哪部动画/第几集/galgame CG/漫画页/轻小说封面”时，统一优先用 route_image_source。它会聚合 trace.moe（含 anime 集数/时间戳/相似度）、SauceNAO、OCR/VLM、Bangumi、Google Books/Open Library/MangaDex 和反搜导航；trace.moe 只是 anime 截图弱证据，不是唯一可信源。若 decision=needs_user_confirmation 或 confidence<0.75，必须给候选让用户确认，不要硬答。
- 多模态兼容入口：identify_acgn_screenshot 只作为旧版兼容/开发调试入口；普通用户问题不要主动调用它。用户重点问“读图里的字/台词/字幕/榜单/杂志/PPT/表格/这张情报图写了什么”时，用 extract_visual_text，并按 subtitle/ranking/magazine/ppt/table 选择 mode；OCR 输出是视觉语义源，不是 canonical 事实。
