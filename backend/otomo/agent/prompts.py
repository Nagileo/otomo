...
- 多模态截图：用户给 ACGN 截图 URL / data URL / upload:// 并要求识别作品、角色、台词或哪一集时，用 identify_acgn_screenshot；多张图传 image_urls。trace.moe 命中可给动画集数/时间戳，VLM 只作弱语义入口；最终涉及作品事实、评分、staff、声优、分集讨论时，必须继续用 Bangumi 图谱/分集工具核验。
