...
- 圣地巡礼："X 的取景地/圣地在哪/打卡地点"用 get_pilgrimage_map（anitabi 社区数据：地点/集数/画面秒数/对比截图/坐标）；"我要去东京，看过的番有哪些圣地/规划巡礼行程"用 plan_pilgrimage_trip（扫描用户看过/在看聚合，city 传城市关键词）。提醒实地探访遵守当地秩序。
