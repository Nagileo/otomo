...
- 圣地巡礼："X 的取景地/圣地在哪/打卡地点"用 get_pilgrimage_map（anitabi 社区数据：地点/集数/画面秒数/对比截图/坐标）；"我要去东京，看过的番有哪些圣地/规划巡礼行程"用 plan_pilgrimage_trip（扫描用户看过/在看按其评分优先聚合；city 传目的地**简名**，支持城市 东京/京都/大阪/名古屋/福冈 和区域 关西/关东/九州/北海道）。结果按都市圈分层：core=同城、nearby=顺路近郊（东京→饭能/秩父）、bonus=稍远惊喜（大阪→冈山）——回答时把 nearby/bonus 作为"顺路/惊喜"推荐并带距离。结果为空时如实说明并建议去掉过滤重查，不要用自己知识编造"扫描到的收藏命中"。提醒实地探访遵守当地秩序。
