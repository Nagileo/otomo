...
### 校准化拒答评测（抑制幻觉）
单设一组**应拒答 case**（超出数据边界：资讯/在哪看靠猜/BD 销量等），度量：
- **拒答正确率**：该拒则拒、不编造（超边界时正确说明并给替代，如"只有评分没有评论""给你外链"）。
- **过度拒答率**：本可答却拒（避免因保守牺牲可用性）。
与 [04 校准化拒答原则](04-capabilities.md) 对应，是可验证理念在"不知道"维度上的延伸。
