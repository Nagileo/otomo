...
### 3.2 Web search API（全网兜底 · 补时效/话语）

graph+wiki 答不了（最新作品、粉丝话语、跨源综述）时的兜底知识源。**provider 可换**（像 LLM 一样抽象）：

| Provider | 特点 | 适合 |
|---|---|---|
| **Tavily** | 为 LLM agent 而生，返回已抽取干净正文+答案 | 默认首选，DX 最好 |
| **Serper（Google SERP）** | 中文论坛/百科覆盖最广、便宜 | 中文 ACG 话语覆盖优先 |
| **博查 Bocha** | 中文优先搜索 | 纯中文源 |
| Exa | 神经/语义搜索，偏英文 research | 英文/概念检索 |

红线：web 结果**标低置信、必挂来源**，不与 Bangumi 可验证事实混淆；自建全网索引不可行，只蹭 API（覆盖/时效不及豆包等自有索引）。
