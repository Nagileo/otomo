...
- **短期记忆（会话）**：跨轮复用同一 `AgentState.messages` → 插在 **API/会话层**（`session_id → AgentState`）。支撑多轮指代（"它的声优呢？"）。已实现。
- **滑动窗口**：上下文过长时按"轮次"安全裁剪（保留 system + 首个问题，且不让窗口以孤立 tool 消息开头）→ 插在 **runner 内、每次 LLM 调用前**（`_common.trim_messages`）。已实现。
- **长期记忆（用户口味画像 / 历史案例）**：跨会话持久化；**任务开始**检索相关记忆注入上下文、**任务结束**写回 → 插在**任务首尾**。属 A4（与口味画像 / RAG 一起做），现留 hook。
