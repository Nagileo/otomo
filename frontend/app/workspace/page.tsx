...
import { authSession, BACKEND, productFetch } from "../../lib/api";
...
export default function WorkspacePage() {
  ...
  useEffect(() => { authSession().then((auth) => { setCsrf(auth.csrf_token || ""); setAuthenticated(Boolean(auth.authenticated)); if (auth.authenticated) void load(); }).catch(() => undefined); }, []);
  ...
  return (
    <main className="page-frame workspace-page">
      ...
      {!authenticated ? <div className="surface-error"><span>工作区需要连接 Bangumi，数据会按账户隔离并跨设备恢复。</span><a className="button-primary" href={`${BACKEND}/auth/bangumi/start`}>立即连接</a></div> : null}
